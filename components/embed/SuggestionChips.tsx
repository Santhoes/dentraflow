...
function getChipClassName(
  ...
  const base =
    "rounded-full px-4 py-2.5 min-h-[44px] text-sm font-medium shrink-0 touch-manipulation transition-transform duration-200 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 ";
  if (item.variant === "danger") {
    return base + "bg-red-600 text-white shadow-sm hover:bg-red-700 hover:shadow-md";
  }
  if (item.variant === "secondary") {
    return (
      base +
      (dark
        ? "border border-slate-500/70 bg-transparent text-slate-200 hover:bg-slate-700/40"
        : "border border-slate-300 bg-white text-slate-700 hover:bg-slate-50")
    );
  }
  if (item.variant === "primary") {
    return base + "text-white shadow-sm hover:shadow-md hover:opacity-95";
  }
  return (
    base +
    (dark
